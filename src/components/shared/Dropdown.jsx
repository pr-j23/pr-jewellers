...
function Dropdown({
  ...
  const hierarchySearchIndex = hierarchicalData?.searchIndex || [];
  const labelLookup = hierarchicalData?.labelLookup || {};

  const [hoveredParentSlug, setHoveredParentSlug] = useState(null);
  const hoverTimeoutRef = useRef(null);
  const hierarchicalSelectedMeta = isHierarchical ? labelLookup[selectedValue] : null;
          ...
          {hierarchySearchResults.length ? (
            hierarchySearchResults.map(result => (
              <li
                ...
                onClick={() => handleHierarchySelection(result)}
              >
                <p className="font-medium text-gray-800">{result.label}</p>
                {result.type === 'child' && (
                  <p className="text-xs text-gray-500">{result.parentName}</p>
                )}
              </li>
            ))
          ) : (
              ...
              <ul className="space-y-1 max-h-64 overflow-y-auto">
                {activeChildren.map(child => (
                  <li key={child.slug}>
                    <button
                      type="button"
                      onClick={() => handleHierarchySelection(labelLookup[child.slug])}
