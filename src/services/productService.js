...
export const getProductRecords = async () => {
  try {
    const response = await getAPI(`/api/tables/${API_CONFIG.tableName}/records`);
    return response;
  } catch (error) {
...
export const deleteProductRecords = async (product_id, successCallBack) => {
  try {
    const response = await deleteAPI(`/api/tables/${API_CONFIG.tableName}/records/${product_id}`);
    if (response?.status === 'success') {
      successCallBack();
    }
    return response;
...
export const editProductRecord = async (productId, productData, toDelete, successCallBack) => {
  const formData = new FormData();
